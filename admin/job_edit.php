...
if ($job_id <= 0) {
    $_SESSION['admin_error'] = 'Invalid job ID specified for edit.';
...
// Fetch existing job
$jQ = mysqli_query($con, "SELECT * FROM jobs WHERE id = $job_id");
$job = mysqli_fetch_assoc($jQ);

if (!$job) {
    $_SESSION['admin_error'] = "Job #$job_id not found.";
    header('Location: jobs.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        $updateQuery = "UPDATE jobs SET 
            company_id=?, company_name=?, company_logo=?, title=?, category=?, job_type=?, experience_level=?, 
            location=?, salary_range=?, openings=?, deadline=?, status=?, description=?, requirements=?, benefits=?, updated_at=NOW() 
            WHERE id=?";
        
        $stmt = mysqli_prepare($con, $updateQuery);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "issssssssisssssi", 
                $company_id, $company_name, $company_logo, $title, $category, $job_type, $experience_level, 
                $location, $salary_range, $openings, $deadline, $status, $description, $requirements, $benefits, $job_id
            );

            if (mysqli_stmt_execute($stmt)) {
                mysqli_stmt_close($stmt);
                $_SESSION['admin_success'] = "Job #$job_id (<strong>" . htmlspecialchars($title) . "</strong>) updated successfully!";
                header('Location: jobs.php');
                exit();
            } else {
    ...
    // Refresh job data if update had error
    $jQ = mysqli_query($con, "SELECT * FROM jobs WHERE id = $job_id");
    $job = mysqli_fetch_assoc($jQ);
}
...
<div class="page-header-box">
    <div class="page-header-title">
        <h1>Edit Job #<?php echo $job_id; ?> ✏️</h1>
        <p>Update posting details, salary, requirements, or publication status.</p>
    </div>
...
<div class="card border-0 rounded-4 shadow-sm mb-4">
    <div class="card-body p-4 p-md-5">
        <form method="POST" action="job_edit.php?id=<?php echo $job_id; ?>">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control border-start-0" name="title" value="<?php echo htmlspecialchars($job['title']); ?>" required>
                    </div>
                </div>
                        ...
                        <select class="form-select border-start-0" name="company_id" id="companySelect">
                            ...
                            <?php if ($companiesRes): ?>
                                <?php while ($comp = mysqli_fetch_assoc($companiesRes)): ?>
                                    <option value="<?php echo $comp['id']; ?>" <?php echo ((int)$job['company_id'] === (int)$comp['id']) ? 'selected' : ''; ?>>
                                        <?php echo htmlspecialchars($comp['name']); ?>
                                    </option>
                                <?php endwhile; ?>
                            <?php endif; ?>
                        </select>
                        <input type="text" class="form-control" name="company_name" value="<?php echo htmlspecialchars($job['company_name']); ?>" placeholder="or enter company name">
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" name="category" required>
                        <option value="Web Development" <?php echo ($job['category'] === 'Web Development') ? 'selected' : ''; ?>>Web Development</option>
                        <option value="Frontend Development" <?php echo ($job['category'] === 'Frontend Development') ? 'selected' : ''; ?>>Frontend Development</option>
                        <option value="Backend Development" <?php echo ($job['category'] === 'Backend Development') ? 'selected' : ''; ?>>Backend Development</option>
                        <option value="UI/UX Design" <?php echo ($job['category'] === 'UI/UX Design') ? 'selected' : ''; ?>>UI/UX Design</option>
                        <option value="DevOps & Cloud" <?php echo ($job['category'] === 'DevOps & Cloud') ? 'selected' : ''; ?>>DevOps & Cloud</option>
                        <option value="Data Science & AI" <?php echo ($job['category'] === 'Data Science & AI') ? 'selected' : ''; ?>>Data Science & AI</option>
                        <option value="Mobile App Development" <?php echo ($job['category'] === 'Mobile App Development') ? 'selected' : ''; ?>>Mobile App Development</option>
                        <option value="Quality Assurance" <?php echo ($job['category'] === 'Quality Assurance') ? 'selected' : ''; ?>>Quality Assurance</option>
                        <option value="Operations & Management" <?php echo ($job['category'] === 'Operations & Management') ? 'selected' : ''; ?>>Operations & Management</option>
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" name="experience_level" required>
                        <option value="Fresher / Entry Level" <?php echo ($job['experience_level'] === 'Fresher / Entry Level' || $job['experience_level'] === 'Fresher') ? 'selected' : ''; ?>>Fresher / Entry Level</option>
                        <option value="1-3 Years" <?php echo ($job['experience_level'] === '1-3 Years') ? 'selected' : ''; ?>>1-3 Years</option>
                        <option value="3-5 Years" <?php echo ($job['experience_level'] === '3-5 Years') ? 'selected' : ''; ?>>3-5 Years</option>
                        <option value="5+ Years" <?php echo ($job['experience_level'] === '5+ Years') ? 'selected' : ''; ?>>5+ Years</option>
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" name="salary_range" value="<?php echo htmlspecialchars($job['salary_range']); ?>" required>
                </div>

                <!-- Openings & Deadline -->
                <div class="col-md-2">
                    <label class="form-label fw-bold text-secondary" style="font-size: 0.85rem;">Openings Count</label>
                    <input type="number" class="form-control" name="openings" value="<?php echo (int)$job['openings']; ?>" min="1">
                </div>

                <div class="col-md-2">
                    <label class="form-label fw-bold text-secondary" style="font-size: 0.85rem;">Application Deadline</label>
                    <input type="date" class="form-control" name="deadline" value="<?php echo htmlspecialchars($job['deadline']); ?>">
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <div class="d-flex gap-3">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status" id="stActive" value="Active" <?php echo ($job['status'] === 'Active') ? 'checked' : ''; ?>>
                            <label class="form-check-label fw-semibold text-success" for="stActive">Active (Visible to Candidates)</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status" id="stDraft" value="Draft" <?php echo ($job['status'] === 'Draft') ? 'checked' : ''; ?>>
                            <label class="form-check-label fw-semibold text-secondary" for="stDraft">Draft (Hidden)</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status" id="stClosed" value="Closed" <?php echo ($job['status'] === 'Closed') ? 'checked' : ''; ?>>
                ...
                <div class="col-md-6">
                    ...
                    <textarea class="form-control" name="requirements" rows="4" required><?php echo htmlspecialchars($job['requirements']); ?></textarea>
                </div>

                <!-- Benefits -->
                <div class="col-md-6">
                    <label class="form-label fw-bold text-secondary" style="font-size: 0.85rem;">Perks & Benefits</label>
                ...
                <div class="col-12 mt-4 pt-3 border-top d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" name="submit" class="btn btn-primary rounded-pill px-5 fw-semibold" style="background: var(--primary-gradient); border: none;">
                        <i class="fas fa-save me-1"></i> Update Job Details
                    </button>
                </div>
